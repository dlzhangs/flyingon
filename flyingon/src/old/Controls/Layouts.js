...
(function (flyingon) {
    ...
    //布局基类
    var layout_base = $class(function (Class, prototype, base) {



        //是否竖排
        prototype.vertical = false;


        //镜像变换类型
        prototype.mirror = "none";
        ...
        flyingon.ready(function () {
            ...
            //竖直滚动条宽度
            prototype.scroll_width = dom.offsetWidth - dom.clientWidth;

            //水平滚动条高度
            prototype.scroll_height = dom.offsetHeight - dom.clientHeight;

            flyingon.distroy(dom);

        });



        prototype[".arrange"] = function (target, width, height) {
        ...
        //排列
        prototype.arrange = function (target, items, width, height) {

        };


        //初始化分隔条
        prototype[".init_splitter"] = function (splitter, width, height, vertical) {
        ...
        //调整控件大小
        prototype[".resize"] = function (target, start, change) {

            var size = start.value + (start.scale === 1 ? change : ((change * start.scale * 100) | 0) / 100);

            if ((size = target[".adjust_size"](size, change, start.vertical)) >= 0)
            {
        ...
        //获取开始调整大小参数
        prototype[".resize_start"] = function (target, vertical, splitter) {

            var style = target[vertical ? "get_height" : "get_width"](),
                value = target[vertical ? "offsetHeight" : "offsetWidth"],
                start = target[".get_unit_scale"](style, value);
        ...
        prototype[".indexOf"] = function (target, x, y) {
        ...
        //获取子控件收拢方向
        prototype[".collapse"] = function (target) {

            return this[".collapse_mirror"](this.vertical ? "top" : "left");
        };


        var reverse = { left: "right", top: "bottom", right: "left", bottom: "top" };

        prototype[".collapse_mirror"] = function (value) {
    ...
    flyingon.defineLayout = function (name, layout_fn) {
        ...
        layout_fn(layout, layout_base.prototype);
    ...
    //流式布局(支持竖排)
    flyingon.defineLayout("flow", function (prototype, base) {
        ...
        prototype[".init_splitter"] = function (splitter, width, height, vertical) {
            ...
            if (target && (target = target.__children[splitter.__arrange_index - 1]))
            {
                ...
                base[".init_splitter"].apply(this, arguments);
            }
        };
    ...
    //线性布局(支持竖排)
    flyingon.defineLayout("line", function (prototype, base) {
    ...
    //3栏布局(支持竖排)
    flyingon.defineLayout("column3", function (prototype, base) {
        ...
        prototype[".resize_start"] = function (target, vertical, splitter) {
            ...
            var start = base[".resize_start"].apply(this, arguments),
        ...
        prototype[".collapse"] = function (target) {
            ...
            return this[".collapse_mirror"](column3);
        };
    ...
    //停靠布局(不支持竖排)
    flyingon.defineLayout("dock", function (prototype, base) {
        ...
        prototype[".resize_start"] = function (target, vertical, splitter) {
            ...
            var mirror = this.mirror,
                ...
                start = base[".resize_start"].call(this, target, vertical = dock === "top" || dock === "bottom", splitter);
        ...
        prototype[".collapse"] = function (target) {
            ...
            return this[".collapse_mirror"](dock !== "none" ? dock : "top");
        };
    ...
    //充满布局(不支持竖排)
    flyingon.defineLayout("fill", function (prototype, base) {
    ...
    //层叠布局(不支持竖排)
    flyingon.defineLayout("cascade", function (prototype, base) {
    ...
    //绝对定位(不支持竖排)
    flyingon.defineLayout("absolute", function (prototype, base) {


        //标记是否绝对定位
        prototype.absolute = true;
    ...
    //页签布局(不支持竖排)
    flyingon.defineLayout("tab", function (prototype, base) {
    ...
    //布局行
    var layout_row = $class(function (Class, prototype) {
        ...
        //计算循环
        prototype.loop = function (loop, back) {
        ...
        //根据子项模板创建子项
        prototype.create = function (length, start, step) {
        ...
        //计算位置及大小
        prototype.compute = function (target, size, spacing) {
    ...
    //网格布局(支持竖排)
    flyingon.defineLayout("grid", function (prototype, base) {
        ...
        prototype.arrange = function (target, items, width, height) {
            ...
            {
                return target[".hide_after"](items, 0);
            }
            ...
            for (var i = 0, _ = items.length; i < _; i++)
            {
                ...
                if ((cache1 = item.get_spacingCells()) > 0 && (index1 += cache1) >= list1.length)
                {
                    ...
                    {
                        return target[".hide_after"](items, i);
                    }
                }
        ...
        prototype[".init_splitter"] = function (splitter, width, height, vertical) {
            ...
            if (splitter.__vertical = splitter.__arrange_column === 0 ||
                splitter[".parent"].__children[splitter.__arrange_index - 1] instanceof flyingon.Splitter ||
        ...
        prototype[".resize"] = function (target, start, change) {
            ...
            if (item)
            {
                ...
                layout_cache = false;
                target[".parent"][vertical ? "set_layoutRows" : "set_layoutColumns"](list.serialize());
            }
        };
        ...
        prototype[".resize_start"] = function (target, vertical, splitter) {
            ...
            var index = (vertical = splitter.__vertical) ? target.__arrange_row : target.__arrange_column,
                ...
                start = target[".get_unit_scale"](item.value + item.unit, item.size);
    ...
    //表格布局(支持竖排)
    flyingon.defineLayout("table", function (prototype, base) {
        ...
        //单元
        var table_cell = $class(function (Class, prototype) {


            //扩展布局格接口
            var copy = layout_cell.call(prototype);


            prototype.copy = function (parent) {
        ...
        //表格行定义
        var table_row = $class(layout_row, function (Class, prototype, base) {
            ...
            //排列控件
            prototype.arrange = function (table, row, items, index, x, y, vertical) {
            ...
            prototype.serialize = function (values) {
        ...
        //表
        var layout_table = $class(layout_row, function (Class, prototype, base) {
            ...
            //解析表格字符串
            prototype.parse = function (tokens, index) {
            ...
            //计算水平排列表大小
            prototype.compute1 = function (target, width, height, spacingWidth, spacingHeight) {
            ...
            //计算竖直排列表大小
            prototype.compute2 = function (target, width, height, spacingWidth, spacingHeight) {
            ...
            //排列控件
            prototype.arrange = function (items, index, x, y, vertical) {
        ...
        //嵌套表
        var cascade_table = $class(layout_table, function (Class, prototype, base) {


            //列间距(仅对子表有效)
            prototype.spacingWidth = "100%";

            //行间距(仅对子表有效)
            prototype.spacingHeight = "100%";
        ...
        prototype[".resize"] = function (target, start, change) {
            ...
            layout_cache = false;
            target[".parent"].set_layoutTable(this.table.serialize());
        };
