...
(function (flyingon) {
    ...
    //获取带前缀的样式名
    flyingon[".style_prefix"] = function (name) {
    ...
    flyingon.dom_user_select = (function () {
        ...
        var style = flyingon[".style_prefix"]("user-select");
    ...
    //样式声明
    flyingon[".declare_style"] = function (defaultWidth, defaultHeight) {
        ...
        function style(name, defaultValue, attributes) {
            ...
            //解析属性
            attributes = _this[".parse.attributes"](attributes);
            ...
            //生成getter,setter方法
            var getter = new Function("var name = '" + key + "', value = this['.styles'] && this['.styles'][name];\n\n"
                    ...
                    + "if ((value = (this.__css_values || flyingon['.compute_css'](this))[name]) !== undefined)\n"
                    + "{\n\t"
                        + "return value;\n"
                    + "}\n\n"

                    + "if (!(name in this.__css_values) && (value = this.__css_values[name] = flyingon['.css_value'](this, name)) !== undefined)\n"
                    + "{\n\t"
                        + "return value;\n"
                    + "}\n\n"

                    + "return " + (attributes.inherit ?
                        "this['.parent'] ? this['.parent'].get_" + key + "() : this.__defaults[name];" :
                        "this.__css_values[name] = this.__defaults[name];\n")),

                setter = _this[".define.setter"](key, style_data_types[key], attributes);
    ...
    flyingon[".compute_css"] = function (target) {
        ...
        else
        {
            ...
            for (var i = 0, _ = names.length; i < _; i++)
            {
                ...
                if ((value = flyingon[".css_value"](target, name)) !== undefined)
                {
    ...
    flyingon[".css_value"] = function (target, name) {
