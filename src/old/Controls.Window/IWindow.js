

//窗口接口
(function (flyingon) {



    //绑定事件方法
    var binding_event = (function () {



        var host = document.documentElement,        //主容器

            events = Object.create(null),           //注册的事件集合

            KeyEvent = flyingon.KeyEvent,
            MouseEvent = flyingon.MouseEvent,

            dragdrop = flyingon.dragdrop,

            draggable,                              //拖动方式
            resizable,                              //调整大小的方式

            setCapture,                             //捕获鼠标
            releaseCapture,                         //释放鼠标
            capture_dom,                            //捕获的dom

            pressdown,                              //按下时dom事件
            pressdown_dom,                          //鼠标按下时dom
            pressdown_target,                       //鼠标按下时目标控件

            hover_control,                          //鼠标指向控件
            host_mousemove = true;                  //是否允许host处理mousemove事件 仅在不能使用setCapture时有效



        //获取dom目标控件
        function dom_target(event, enabled) {

            var target = event.target;

            while (target)
            {
                if (target.flyingon)
                {
                    target = target.flyingon;

                    if (enabled !== false)
                    {
                        while (!target.get_enabled())
                        {
                            target = target[".parent"];
                        }
                    }

                    return target || host.flyingon;
                }

                target = target.parentNode
            }

            return host.flyingon;
        };



        //捕获或释放鼠标
        if (document.createElement("div").setCapture)
        {

            setCapture = function (dom) {

                flyingon.dom_user_select(document.body, false);

                if (capture_dom = dom)
                {
                    dom.setCapture();
                    //dom.onlosecapture = pressdown_cancel; //IE特有 注册此方法会造成IE7无法拖动滚动条
                }
            };

            releaseCapture = function () {

                flyingon.dom_user_select(document.body, true);

                capture_dom.releaseCapture();
                //capture_dom.onlosecapture = null;
            };

        }
        else //window.captureEvents方式效果不好,不要使用
        {

            //设置捕获
            setCapture = function (dom) {

                flyingon.dom_user_select(document.body, false);
            };


            //释放捕获
            releaseCapture = function () {

                flyingon.dom_user_select(document.body, true);
            };


            //捕获全局mousemove事件
            flyingon.on(host, "mousemove", function (event) {

                if (!host_mousemove)
                {
                    host_mousemove = true;
                }
                else if (pressdown)
                {
                    events.mousemove.call(this, event);
                }
                else if (hover_control)
                {
                    hover_control.trigger(new MouseEvent("mouseout", event, pressdown), true);
                    hover_control[".stateTo_hover"](false);
                }
            });


        };




        //捕获全局mouseup事件
        //注: 当鼠标在浏览器窗口外弹起时此事件仍不能执行,暂时找不到好的办法捕获浏览器外mouseup事件
        //IE的onlosecapture会造成IE7等浏览器滚动条不能拖动
        //window的onblur事件在IE7等浏览器中性能太差
        flyingon.on(host, "mouseup", function (event) {

            if (pressdown)
            {
                events.mouseup.call(this, event, true);
            }
        });



        //解决某些情况下鼠标按下拖动后无法执行mouseup的问题 IE此用此方法在拖动滚动条时性能很差
        //flyingon.on(window, "blur", pressdown_cancel);





        //注:IE6/7/8的鼠标事件的event中鼠标数据是全局的,不能直接记录,需要把相关的数据存储至pressdown对象中
        events.mousedown = function (event) {

            //如果因为移出窗口等原因丢失mouseup事件则触发mouseup
            if (pressdown)
            {
                events.mouseup(event);
            }
            else
            {
                var ownerWindow = this.flyingon,
                    target,
                    cache;

                //活动窗口不是当前点击窗口则设置为活动窗口
                if (ownerWindow.__mainWindow.__activeWindow !== ownerWindow)
                {
                    ownerWindow.active();
                    host.flyingon = ownerWindow;
                }

                //鼠标按键处理
                //IE678 button: 1->4->2 W3C button: 0->1->2
                //本系统统一使用which 左中右 1->2->3
                if (event.which === undefined)
                {
                    event.which = event.button & 1 ? 1 : (event.button & 2 ? 3 : 2);
                }

                //获取enabled控件
                target = cache = dom_target(event, false) || ownerWindow;

                while (!target.get_enabled())
                {
                    if (!(target = target[".parent"]))
                    {
                        target = ownerWindow;
                        break;
                    }
                }

                //初始化鼠标按下时数据
                pressdown = {

                    capture: false,
                    dom: pressdown_dom = event.target,
                    which: event.which,
                    clientX: event.clientX,
                    clientY: event.clientY,
                    target: pressdown_target = target
                };

                //分发mousedown事件
                target.trigger(new MouseEvent("mousedown", event));

                //可调整大小 触控版目前不支持调整大小
                if (resizable)
                {
                    //设置目标控件
                    pressdown.target = resizable.target;
                    pressdown.capture = true;

                    //禁止点击事件
                    flyingon.__disable_click = flyingon.__disable_dbclick = true;
                }
                else
                {
                    //检测拖动
                    while (cache)
                    {
                        if ((draggable = cache.get_draggable()) !== "none" && (draggable = cache[".check_drag"](draggable, event))) //可拖动
                        {
                            if (dragdrop.start(cache, draggable, event))
                            {
                                //设置捕获目标
                                pressdown.target = cache;
                                pressdown.capture = true;

                                return;
                            }
                        }

                        cache = cache[".parent"];
                    }

                    //清除拖动状态
                    draggable = null;

                    //设置活动状态
                    target[".stateTo_active"](true);
                }
            }
        };


        events.mousemove = function (event) {

            var ownerWindow = this.flyingon,
                target,
                cache;

            //防止host处理
            host_mousemove = false;

            if (pressdown)
            {
                if (resizable) //调整大小
                {
                    if (target = pressdown.target)
                    {
                        target[".resize"](resizable, event, pressdown);
                        ownerWindow[".registry_update"](target, true);
                    }
                }
                else if (draggable) //拖动
                {
                    dragdrop.move(event, pressdown);
                }
                else if (target = pressdown_target)  //启用捕获
                {
                    target.trigger(new MouseEvent("mousemove", event, pressdown));
                }

                //捕获dom(只能捕获当前事件dom,不能捕获target.dom,否则在两个dom不同的情况下IE会造成滚动条无法拖动的问题)
                if (!capture_dom && pressdown.capture)
                {
                    setCapture(pressdown_dom);
                }
            }
            else if (cache = target = dom_target(event, false))
            {
                //调整大小状态不触发相关事件
                while (cache)
                {
                    if ((resizable = cache.get_resizable()) !== "none" && (resizable = cache[".check_resize"](resizable, event)))
                    {
                        resizable.target = cache;
                        ownerWindow[".set_cursor"](cache, resizable.cursor);
                        return;
                    }

                    cache = cache[".parent"];
                }

                resizable = null;

                //获取enabled控件
                while (!target.get_enabled())
                {
                    if (!(target = target[".parent"]))
                    {
                        target = ownerWindow;
                        break;
                    }
                }

                if (target !== (cache = hover_control))
                {
                    if (cache)
                    {
                        cache.trigger(new MouseEvent("mouseout", event, pressdown), true);

                        while (cache)
                        {
                            cache[".stateTo_hover"](false);
                            cache = cache[".parent"];
                        }
                    }

                    hover_control = target;

                    target.trigger(new MouseEvent("mouseover", event, pressdown));

                    cache = target;

                    while (cache)
                    {
                        cache[".stateTo_hover"](true);
                        cache = cache[".parent"];
                    }
                }

                target.trigger(new MouseEvent("mousemove", event, pressdown));

                ownerWindow[".set_cursor"](target);
            }
        };


        events.mouseup = function (event, cancel) {

            var target;

            if (capture_dom)
            {
                releaseCapture();
                capture_dom = null;
            }

            if (resizable)
            {
                resizable = null;
            }

            if (draggable) //如果处于拖动状态则停止拖动
            {
                if (dragdrop.stop(event, pressdown, cancel)) //如果拖动过则取消相关鼠标事件
                {
                    flyingon.__disable_click = flyingon.__disable_dbclick = true; //禁止点击事件
                }

                draggable = null;
            }

            if (pressdown && (target = pressdown_target))
            {
                target.trigger(new MouseEvent("mouseup", event, pressdown));
                target[".stateTo_active"](false); //取消活动状态
            }

            pressdown = null;
        };


        //dom鼠标事件顺序: mousedown -> mouseup -> click -> mousedown -> mouseup -> click -> dblclick
        events.click = function (event) {

            var target;

            if (flyingon.__disable_click)
            {
                flyingon.__disable_click = false;
            }
            else if (target = dom_target(event))
            {
                return target.trigger(new MouseEvent("click", event));
            }
        };


        events.dblclick = function (event) {

            var target;

            if (flyingon.__disable_dbclick)
            {
                flyingon.__disable_dbclick = false;
            }
            else if (target = dom_target(event))
            {
                return target.trigger(new MouseEvent("dblclick", event));
            }
        };


        //DOMMouseScroll:firefox滚动事件
        events.mousewheel = events.DOMMouseScroll = function (event) {

            var target = dom_target(event),
                value = event.wheelDelta || (-event.detail * 40);//鼠标滚轮数据

            event = new MouseEvent("mousewheel", event, pressdown);
            event.wheelDelta = value;

            return target.trigger(event);
        };



        //events.touchstart = function(event) {


        //};

        //events.touchmove = function(event) {


        //};

        //events.touchend = function(event) {


        //};

        //events.touchcancel = function(event) {


        //};


        events.keydown = events.keypress = events.keyup = function (event) {

            //按键判断统一使用which
            if (!event.which)
            {
                event.which = event.charCode || event.keyCode;
            }

            return dom_target(event).trigger(new KeyEvent(event.type, event));
        };


        events.contextmenu = function (event) {

            return dom_target(event).trigger("contextmenu");
        };


        events.load = function (event) {

            return dom_target(event).trigger("contextmenu");
        };


        events.error = function (event) {

            return dom_target(event).trigger("contextmenu");
        };




        //直接绑定事件至dom, 解决某些事件(如scroll)无法在父控件正确捕获的问题
        flyingon[".dom_event"] = function (target, release) {

            var dom = target.dom;

            (target.dom_children && target.dom_children.parentNode || dom).onscroll = release ? null : onscroll;

            dom.onfocus = release ? null : onfocus;
            dom.onblur = release ? null : onblur;

            target.__has_dom_event = true;
        };



        function onscroll(event) {

            var dom = event.target,
                target = dom_target(event),
                result;

            target.__scrollLeft = dom.scrollLeft;
            target.__scrollTop = dom.scrollTop;

            if (target[".on_scroll"])
            {
                target[".on_scroll"](event);
            }

            result = target.trigger("scroll");

            target.__scrollLeft_last = dom.scrollLeft;
            target.__scrollTop_last = dom.scrollTop;

            pressdown = null; //IE滚动时无法触发mouseup事件

            return result;
        };


        function onfocus(event) {

            var target = dom_target(event);

            return target.trigger("focus");
        };


        function onblur(event) {

            var target = dom_target(event);


            return target.trigger("blur");
        };



        //初始化绑定事件方法
        return function (dom) {

            for (var name in events)
            {
                flyingon.on(dom, name, events[name]); //直接绑定至dom的on事件以提升性能
            }
        };



    })();



    //延时更新器
    function delay_update(ownerWindow) {


        var target, timer, callback_fn;


        //更新控件样式
        function update() {

            if (callback_fn)
            {
                callback_fn(ownerWindow);
                callback_fn = null;
            }

            if (target)
            {
                target.render();
                target = null;
            }

            timer = 0;
        };


        //注册控件更新
        ownerWindow[".registry_update"] = function (control, update_now, callback) {

            if (timer)
            {
                clearTimeout(timer);
            }

            if (callback && !callback_fn) //如果有回调函数 只有本次刷新前第一个注册的回调函数才可被执行
            {
                callback_fn = callback;
            }

            if (target)
            {
                if (control !== target && control[".parent"] !== target)
                {
                    target = target[".parent"] !== control ? ownerWindow : control;
                }
            }
            else
            {
                target = control;
            }

            if (update_now)
            {
                update();
                timer = 0;
            }
            else
            {
                timer = setTimeout(update, 20);
            }
        };

    };



    //窗口集合
    flyingon.__all_windows = [];



    //窗口接口
    flyingon.IWindow = function (prototype, base) {


        prototype[".init_window"] = function (dom) {


            //标记所属窗口为自身及绑定dom对象
            this.__ownerWindow = dom.flyingon = this;

            //绑定事件       
            binding_event(dom);

            //注册自动更新服务
            delay_update(this);

        };



        //主窗口
        flyingon.defineProperty(prototype, "mainWindow", function () {

            return this.__mainWindow || null;
        });


        //活动窗口
        flyingon.defineProperty(prototype, "activeWindow", function () {

            return this.__mainWindow && this.__mainWindow.__activeWindow || null;
        });


        //父窗口
        flyingon.defineProperty(prototype, "parentWindow", function () {

            return this.__parentWindow || null;
        });




        //修改透明度属性
        prototype.defineProperty("opacity", 1, {

            minValue: 0,
            maxValue: 1,
            set_code: "this.dom.style.opacity = value;"
        });



        //设置当前窗口为活动窗口
        prototype.active = function () {

            var first = this.get_mainWindow(),
                target;

            if ((target = first.__activeWindow) !== this)
            {
                if (target)
                {
                    if (target !== first)
                    {
                        target.dom.style.zIndex = 0;
                    }

                    target.trigger("deactivate");
                    target[".stateTo_active"](false);
                }

                this.trigger("activate");

                first.__activeWindow = this;

                if (this !== first)
                {
                    this.dom.style.zIndex = 1;
                }

                this[".stateTo_active"](true);
            }
        };




        var style;

        //设置光标
        prototype[".set_cursor"] = function (target, cursor) {

            if (style) //清除原有样式
            {
                style.cursor = "";
            }

            //同时设置目标控件及主窗口dom_window的光标
            if (target)
            {
                style = target.dom.style;
                style.cursor = this.__mainWindow.dom_window.style.cursor = cursor || (target.get_draggable() !== "none" ? "move" : target.get_cursor());
            }
        };



    };



})(flyingon);


